...
function doRename(basePath, oldName, newName) {
    return fs_extra_1.rename(path.join(basePath, oldName), path.join(basePath, newName));
}
...
async function createMacApp(packager, appOutDir, asarIntegrity, isMas) {
    ...
    await fs_extra_1.rename(path.dirname(contentsPath), appPath);
    // https://github.com/electron-userland/electron-builder/issues/840
    const now = Date.now() / 1000;
    await fs_extra_1.utimes(appPath, now, now);
}
