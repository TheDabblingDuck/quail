...
class LibUiFramework {
    ...
    async prepareMacosApplicationStageDirectory(packager, options) {
        ...
        await fs_extra_1.ensureDir(path.join(appContentsDir, "Resources"));
        await fs_extra_1.ensureDir(path.join(appContentsDir, "MacOS"));
  ...
  `);
    ...
    async afterPack(context) {
        ...
        await fs_extra_1.rename(path.join(context.appOutDir, "app", userMain), path.join(context.appOutDir, "app", "main.js"));
    }
...
async function writeExecutableMain(file, content) {
    await fs_extra_1.writeFile(file, content, { mode: 0o755 });
    await fs_extra_1.chmod(file, 0o755);
}
