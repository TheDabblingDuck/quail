...
class AppPackageHelper {
    ...
    async finishBuild() {
        ...
        await bluebird_lst_1.default.map(filesToDelete, it => fs_extra_1.unlink(it));
    }
}
...
class UninstallerReader {
    ...
    static exec(installerPath, uninstallerPath) {
        const buffer = fs_2.default.readFileSync(installerPath);
        ...
        while (true) {
            ...
            if (compressed) {
                buffer = zlib_1.default.inflateRawSync(buffer);
            }
        ...
        fs_2.default.writeFileSync(uninstallerPath, executable);
        fs_2.default.appendFileSync(uninstallerPath, innerBuffer);
    }
}
