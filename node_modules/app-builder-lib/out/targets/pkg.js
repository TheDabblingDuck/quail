...
class PkgTarget extends core_1.Target {
    ...
    async build(appPath, arch) {
        ...
        await Promise.all([fs_extra_1.unlink(innerPackageFile), fs_extra_1.unlink(distInfoFile)]);
    ...
    async customizeDistributionConfiguration(distInfoFile, appPath) {
        ...
        const options = this.options;
        let distInfo = await fs_extra_1.readFile(distInfoFile, "utf-8");
        ...
        await fs_extra_1.writeFile(distInfoFile, distInfo);
    }
