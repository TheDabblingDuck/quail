...
const projectTemplate = new lazy_val_1.Lazy(async () => {
    const template = (await fs_extra_1.readFile(path.join(pathManager_1.getTemplatePath("msi"), "template.xml"), "utf8"))
        .replace(/{{/g, "<%")
        .replace(/}}/g, "%>")
        .replace(/\${([^}]+)}/g, "<%=$1%>");
...
class MsiTarget extends core_1.Target {
    ...
    async build(appOutDir, arch) {
        ...
        await fs_extra_1.writeFile(projectFile, await this.writeManifest(appOutDir, arch, commonOptions));
        if (uiFile !== null) {
            await fs_extra_1.writeFile(uiFile, await fs_extra_1.readFile(path.join(pathManager_1.getTemplatePath("msi"), ASSISTED_UI_FILE_NAME), "utf8"));
