...
class AsarPackager {
    ...
    async pack(fileSets, packager) {
        ...
        await fs_extra_1.ensureDir(path.dirname(this.outFile));
        ...
        const correctDirNodeUnpackedFlag = async (filePathInArchive, dirNode) => {
            for (const dir of unpackedDirs) {
                if (filePathInArchive.length > dir.length + 2 && filePathInArchive[dir.length] === path.sep && filePathInArchive.startsWith(dir)) {
                    ...
                    await fs_extra_1.ensureDir(path.join(this.unpackedDest, filePathInArchive));
                    break;
                }
            }
        };
        ...
        for (let i = 0, n = fileSet.files.length; i < n; i++) {
            ...
            if (isUnpacked) {
                if (!dirNode.unpacked && !dirToCreateForUnpackedFiles.has(fileParent)) {
                    ...
                    await fs_extra_1.ensureDir(path.join(this.unpackedDest, fileParent));
                }
    ...
    writeAsarFile(fileSets, unpackedFileIndexMap) {
        return new Promise((resolve, reject) => {
            ...
            const writeStream = fs_extra_1.createWriteStream(this.outFile);
            ...
            const w = (index) => {
                ...
                if (stat != null && stat.size < 2 * 1024 * 1024) {
                    ...
                    const readStream = fs_extra_1.createReadStream(file);
...
async function order(filenames, orderingFile, src) {
    const orderingFiles = (await fs_extra_1.readFile(orderingFile, "utf8")).split("\n").map(line => {
        if (line.indexOf(":") !== -1) {
            line = line.split(":").pop();
        }
        line = line.trim();
...
function copyFileOrData(fileCopier, data, source, destination, stats) {
    if (data == null) {
        ...
        return fs_extra_1.writeFile(destination, data);
    }
}
